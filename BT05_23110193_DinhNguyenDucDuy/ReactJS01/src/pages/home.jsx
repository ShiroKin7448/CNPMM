...
import { getHomeProductsApi, getCategoriesApi, getTopProductsApi } from "../util/api.js";
...
const HomePage = () => {
  ...
  const [topProducts, setTopProducts] = useState({ bestSelling: [], mostViewed: [] });
  ...
  useEffect(() => {
    ...
    Promise.all([
      getHomeProductsApi(),
      getCategoriesApi(),
      getTopProductsApi({ type: "best-selling", limit: 10 }),
      getTopProductsApi({ type: "most-viewed", limit: 10 }),
    ]).then(([home, cats, bestSelling, mostViewed]) => {
      if (home?.EC === 0) setData(home.DT);
      if (cats?.EC === 0) setCategories(cats.DT);
      setTopProducts({
        bestSelling: bestSelling?.EC === 0 ? bestSelling.DT.products : [],
        mostViewed: mostViewed?.EC === 0 ? mostViewed.DT.products : [],
      });
      setLoading(false);
    });
    ...
    <div className="min-h-screen bg-transparent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {loading ? (
          ...
          <>
            <TopProductsCarousel type="best-selling" products={topProducts.bestSelling} />
            <TopProductsCarousel type="most-viewed" products={topProducts.mostViewed} />
