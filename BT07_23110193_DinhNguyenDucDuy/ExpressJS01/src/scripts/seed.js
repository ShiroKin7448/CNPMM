...
const seed = async () => {
  ...
  const users = await User.insertMany([
    {
      ...
      loyaltyTransactions: [
        { type: "PURCHASE_REWARD", points: 320, description: "Tích điểm mua hàng từ đơn LSBT07DEMO001", reference: "LSBT07DEMO001" },
  ...
  const demoOrder = await Order.create({
    ...
    timeline: [
      { status: "NEW", label: "Đơn hàng mới", note: "Đơn demo BT07", at: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000) },
  ...
  console.log(`✅ Seeded BT07: ${productsWithSlug.length} products, ${cats.length} categories, ${users.length} users, 3 vouchers, 1 delivered order & 1 review`);
