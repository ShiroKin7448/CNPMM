...
import { FiCpu, FiEye, FiHardDrive, FiHeart, FiMessageCircle, FiShoppingBag } from "react-icons/fi";
import { HiStar } from "react-icons/hi";
import { useAuth } from "../context/auth.context.jsx";
import { toggleFavoriteApi } from "../../util/api.js";
...
const ProductCard = ({ product }) => {
  ...
  const toggleFavorite = async (event) => {
    ...
    try {
      ...
      message.error(error?.EM || "Không thể cập nhật yêu thích");
    }
  };
  ...
  return (
    <article
      onClick={goToDetail}
      className="product-card-3d group flex h-full cursor-pointer flex-col overflow-hidden rounded-[26px] border border-[#D5D5D5] bg-white shadow-sm shadow-black/5 transition-all duration-300 hover:border-black hover:shadow-2xl hover:shadow-black/18"
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-[#D5D5D5]">
        ...
        <button
          ...
          className={`absolute right-3 top-3 flex h-10 w-10 items-center justify-center rounded-full border border-white/70 shadow-lg shadow-black/15 backdrop-blur transition hover:bg-[#C0FF6B] ${favorite ? "bg-[#C0FF6B] text-black" : "bg-white/90 text-black"}`}
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-3 grid min-h-[42px] grid-cols-2 gap-2">
          {specs.length > 0
            ? specs.map((spec) => (
              <div
                key={spec.text}
            ...
            : <span className="col-span-2 rounded-2xl border border-transparent px-2.5 py-2 text-[11px] font-bold text-transparent">Thông số</span>
          }
        </div>
        ...
        <div className="mt-3 flex min-h-[26px] flex-wrap items-center gap-2">
          ...
          {typeof product.viewCount === "number" && (
            <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#656565]">
              <FiEye size={13} />
              <b className="text-black">{product.viewCount.toLocaleString("vi-VN")}</b>
            </span>
          )}
          <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#656565]">
            <FiMessageCircle size={13} />
            <b className="text-black">{product.commentCustomerCount || product.reviewCount || 0}</b>
          </span>
        </div>

        <div className="mt-auto pt-4">
          <div className="product-price-block">
            <span className="product-price-main">{formatPrice(activePrice)}</span>
            {product.salePrice && (
