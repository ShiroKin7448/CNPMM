...
import {
  DashboardOutlined, HomeOutlined, UserOutlined, LoginOutlined, LogoutOutlined,
  TeamOutlined, AppstoreOutlined, KeyOutlined, ShoppingOutlined,
  ShoppingCartOutlined, OrderedListOutlined, GiftOutlined, HeartOutlined,
} from "@ant-design/icons";
import { FiUser } from "react-icons/fi";
import { useAuth } from "../context/auth.context.jsx";
import { getCartApi } from "../../util/api.js";
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        <Link to={homePath} className="site-logo-3d flex items-center gap-2.5 no-underline group">
          <div className="logo-cube-3d w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-md shadow-indigo-200 group-hover:scale-110 transition-transform">
          ...
          <div>
            ...
            <span className="hidden sm:inline text-xs text-slate-400 ml-1.5 font-medium">BT07</span>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navItems.map(item => {
            const isActive = location.pathname === item.key ||
              ...
              (item.key === "/orders" && location.pathname.startsWith("/orders")) ||
            ...
            <>
              ...
              <Dropdown menu={dropdownItems} placement="bottomRight" arrow trigger={["click"]}>
                <button className="flex items-center gap-2.5 bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded-xl px-3 py-2 cursor-pointer transition-all">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-semibold hidden sm:inline ${isAdmin ? "bg-amber-100 text-amber-700" : "bg-indigo-100 text-indigo-700"}`}>
                    {isAdmin ? "Admin" : "User"}
                  </span>
                </button>
              </Dropdown>
