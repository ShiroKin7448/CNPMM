...
import { FiArrowLeft, FiCheck, FiClock, FiCreditCard, FiExternalLink, FiMessageSquare, FiPackage, FiRefreshCw, FiTruck, FiXCircle } from "react-icons/fi";
                    ...
                    <div key={`${order._id}-${item.product}`} className="grid grid-cols-[72px_1fr_auto] gap-3 rounded-2xl border border-[#D5D5D5] p-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {order.status === "DELIVERED" && (
                          <Link to={`/product/${item.product}?review=1`} className="mt-2 inline-flex items-center gap-1 text-xs font-extrabold text-black underline">
                            <FiMessageSquare /> Bình luận và nhận quà
                          </Link>
                        )}
                      </div>
            ...
            <aside className="h-fit rounded-3xl border border-[#D5D5D5] bg-white p-5 shadow-sm lg:sticky lg:top-24">
              ...
              <div className="space-y-3 border-b border-[#D5D5D5] pb-5 text-sm font-bold text-[#656565]">
                ...
                {order.pointsDiscount > 0 && (
                  <div className="flex justify-between text-emerald-700">
                    <span>Đổi {order.pointsUsed} điểm</span>
              ...
              {order.loyaltyPointsEarned > 0 && (
                <div className="mt-3 rounded-xl bg-[#C0FF6B]/30 p-3 text-xs font-extrabold text-black">
                  Đơn giao thành công đã cộng {order.loyaltyPointsEarned} điểm vào kho tích lũy.
                </div>
              )}
