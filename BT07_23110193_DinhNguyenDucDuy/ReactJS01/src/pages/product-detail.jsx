...
import { FiEye, FiHeart, FiMessageCircle, FiPackage, FiShield, FiShoppingCart, FiTruck, FiUsers } from "react-icons/fi";
...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    if (auth.isAuthenticated) recordRecentlyViewedApi(id).catch(() => {});
  ...
  const handleAddToCart = async () => {
    ...
    setAdding(true);
    try {
      const res = await addCartItemApi(product._id, qty);
      if (res?.EC === 0) {
      ...
      message.error(error?.EM || "Không thể thêm vào giỏ hàng");
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleFavorite = async () => {
    ...
    try {
      ...
      setFavorite(Boolean(res?.DT?.isFavorite));
      message.success(res.EM);
    } catch (error) {
      message.error(error?.EM || "Không thể cập nhật sản phẩm yêu thích");
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="fade-in-up" style={{ animationDelay: "0.1s" }}>
            ...
            <div className="mb-5 grid grid-cols-3 gap-2">
              <div className="rounded-xl bg-[#f6f6f6] p-3 text-center text-xs font-bold text-[#656565]">
                <FiUsers className="mx-auto mb-1 text-black" /> <b className="text-black">{product.buyerCount}</b> khách mua
              </div>
              <div className="rounded-xl bg-[#f6f6f6] p-3 text-center text-xs font-bold text-[#656565]">
                <FiMessageCircle className="mx-auto mb-1 text-black" /> <b className="text-black">{product.commentCustomerCount}</b> khách bình luận
              </div>
              <div className="rounded-xl bg-[#f6f6f6] p-3 text-center text-xs font-bold text-[#656565]">
                <FiEye className="mx-auto mb-1 text-black" /> <b className="text-black">{product.viewCount}</b> lượt xem
              </div>
            </div>
            ...
            {inStock && (
              <div className="flex items-center gap-4 mb-6">
                ...
                <button
                  ...
                  disabled={adding}
                  className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all hover:shadow-lg hover:shadow-indigo-200 active:scale-95 disabled:opacity-60"
                >
                  <FiShoppingCart size={18} /> {adding ? "Đang thêm..." : "Thêm Vào Giỏ"}
                </button>
                <button
                  type="button"
                  onClick={handleFavorite}
                  className={`grid h-12 w-12 place-items-center rounded-xl border transition ${favorite ? "border-black bg-[#C0FF6B] text-black" : "border-[#D5D5D5] bg-white text-[#656565] hover:border-black"}`}
                  aria-label="Yêu thích"
                >
                  <FiHeart size={18} fill={favorite ? "currentColor" : "none"} />
                </button>
              </div>
            )}
