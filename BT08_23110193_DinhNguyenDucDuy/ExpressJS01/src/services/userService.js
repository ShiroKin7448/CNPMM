...
import { notifyNewUser } from "./notificationService.js";
dotenv.config();
  ...
  try {
    ...
    try {
      ...
      await transporter.sendMail({
        ...
        to: normalizedEmail,
        subject: "Xác nhận tài khoản - LaptopStore BT07",
        html: `
          <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
            ...
            <div style="background:#fff;padding:30px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;">
              ...
              <p style="color:#9ca3af;font-size:12px;text-align:center;">© 2026 BT07 - 23110193 - Đinh Nguyễn Đức Duy</p>
            </div>
          </div>
        `,
      });
    ...
    runNotification(notifyNewUser(newUser));
...
const forgotPasswordService = async (email) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL_FROM,
      to: user.email,
      subject: "Yêu cầu đặt lại mật khẩu - LaptopStore BT07",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <p style="color: #999; font-size: 12px; text-align: center;">
            © 2026 BT07 - 23110193 - Đinh Nguyễn Đức Duy
          </p>
        </div>
      `,
    };
...
const resendVerificationService = async (email) => {
  try {
    ...
    await transporter.sendMail({
      from: process.env.EMAIL_FROM,
      to: normalizedEmail,
      subject: "Gửi lại xác nhận tài khoản - LaptopStore BT07",
