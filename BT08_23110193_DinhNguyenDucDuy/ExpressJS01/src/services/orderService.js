...
import {
  awardDeliveryLoyaltyService,
  commitOrderBenefitsService,
  prepareCheckoutBenefitsService,
  restoreOrderBenefitsService,
} from "./loyaltyService.js";
import {
  emitAdminAnalyticsRefresh,
  notifyCancelRequest,
  notifyNewOrder,
...
const getCancelPolicy = (order) => {
  if (!order || [ORDER_STATUS.CANCELLED, ORDER_STATUS.DELIVERED, ORDER_STATUS.SHIPPING].includes(order.status)) {
    return {
      canCancel: false,
      action: "none",
      title: "KhÃ´ng thá»ƒ há»§y Ä‘Æ¡n",
      description: "ÄÆ¡n hÃ ng hiá»‡n táº¡i khÃ´ng cÃ²n náº±m trong tráº¡ng thÃ¡i cho phÃ©p há»§y.",
    };
  }
  ...
  if (isMomoPaid(order) && [ORDER_STATUS.NEW, ORDER_STATUS.CONFIRMED, ORDER_STATUS.PREPARING].includes(order.status)) {
    return {
      canCancel: true,
      action: "request",
      title: "Gá»­i yÃªu cáº§u há»§y vÃ  hoÃ n tiá»n",
      description: "ÄÆ¡n Ä‘Ã£ thanh toÃ¡n qua MoMo nÃªn shop cáº§n duyá»‡t há»§y vÃ  thá»±c hiá»‡n hoÃ n tiá»n trÃªn MoMo sandbox.",
    };
  }
  ...
  if (order.status === ORDER_STATUS.PREPARING) {
    return {
      canCancel: true,
      action: "request",
      title: "Gá»­i yÃªu cáº§u há»§y",
      description: "Shop Ä‘ang chuáº©n bá»‹ hÃ ng, yÃªu cáº§u há»§y cáº§n admin/shop duyá»‡t.",
    };
  }
  ...
  if (canCancelByTime(order)) {
    return {
      canCancel: true,
      action: "direct",
      title: "Há»§y trá»±c tiáº¿p",
      description: "ÄÆ¡n chÆ°a thu tiá»n vÃ  cÃ²n trong 30 phÃºt Ä‘áº§u, há»‡ thá»‘ng sáº½ há»§y ngay sau khi xÃ¡c nháº­n.",
    };
  }
  ...
  return {
    ...
    action: "none",
    title: "KhÃ´ng thá»ƒ há»§y Ä‘Æ¡n",
    description: "Sau 30 phÃºt hoáº·c khi Ä‘Æ¡n Ä‘Ã£ sang bÆ°á»›c xá»­ lÃ½ tiáº¿p theo, ngÆ°á»i dÃ¹ng khÃ´ng thá»ƒ tá»± há»§y Ä‘Æ¡n.",
  };
};
...
const autoConfirmOrder = async (order) => {
  ...
  order.timeline.push(createTimelineEntry(
    ...
    "Há»‡ thá»‘ng tá»± Ä‘á»™ng xÃ¡c nháº­n sau 30 phÃºt.",
    deadline
  ));
  await order.save();
  return order;
...
const reserveStock = async (items = []) => {
  ...
  for (const item of items) {
    ...
    if (!updatedProduct) {
      ...
      return {
        ok: false,
        message: `${item.name} khÃ´ng cÃ²n Ä‘á»§ ${item.quantity} sáº£n pháº©m trong kho`,
      };
    }
...
const validateShippingAddress = (shippingAddress = {}) => {
  if (isBlank(shippingAddress.fullName)) return "Vui lÃ²ng nháº­p há» tÃªn ngÆ°á»i nháº­n";
  if (isBlank(shippingAddress.phone)) return "Vui lÃ²ng nháº­p sá»‘ Ä‘iá»‡n thoáº¡i";
  if (isBlank(shippingAddress.address)) return "Vui lÃ²ng nháº­p Ä‘á»‹a chá»‰ giao hÃ ng";
  return "";
};
...
const normalizePaymentMethod = (paymentMethod) => {
  ...
  if (![PAYMENT_METHOD.COD, PAYMENT_METHOD.MOMO].includes(method)) {
    return { error: "Chá»‰ há»— trá»£ thanh toÃ¡n COD hoáº·c MoMo sandbox" };
  }
  return { method };
};
...
const markMomoCreateFailed = async (order, message) => {
  ...
  order.cancelReason = message || "KhÃ´ng táº¡o Ä‘Æ°á»£c giao dá»‹ch MoMo sandbox.";
  ...
  await restoreOrderBenefitsService(order);
};
...
const applyPaidMomoResult = async (order, payload) => {
  ...
  if (paidAmount !== Math.round(order.total)) {
    order.timeline.push(createTimelineEntry(
      order.status,
      `MoMo tráº£ vá» sá»‘ tiá»n ${paidAmount.toLocaleString("vi-VN")} VND khÃ´ng khá»›p tá»•ng Ä‘Æ¡n ${Math.round(order.total).toLocaleString("vi-VN")} VND.`
    ));
    await order.save();
    return { EC: 1, EM: "Sá»‘ tiá»n MoMo tráº£ vá» khÃ´ng khá»›p Ä‘Æ¡n hÃ ng", DT: decorateOrder(order) };
  }
  ...
  if (order.paymentStatus === "PAID" && (!incomingTransId || incomingTransId === order.paymentInfo?.transId)) {
    return { EC: 0, EM: "ÄÆ¡n Ä‘Ã£ Ä‘Æ°á»£c xÃ¡c nháº­n thanh toÃ¡n MoMo", DT: decorateOrder(order) };
  }
  ...
  if (order.status === ORDER_STATUS.CANCELLED) {
    ...
    order.timeline.push(createTimelineEntry(
      ...
      "MoMo bÃ¡o thanh toÃ¡n thÃ nh cÃ´ng sau khi Ä‘Æ¡n Ä‘Ã£ há»§y. Cáº§n admin hoÃ n tiá»n thá»§ cÃ´ng qua MoMo."
    ));
  } else {
    order.paymentStatus = "PAID";
    order.timeline.push(createTimelineEntry(
      order.status,
      `MoMo sandbox xÃ¡c nháº­n Ä‘Ã£ thu ${Math.round(order.total).toLocaleString("vi-VN")} VND.`
    ));
  }

  await order.save();
  return { EC: 0, EM: "XÃ¡c nháº­n thanh toÃ¡n MoMo thÃ nh cÃ´ng", DT: decorateOrder(order) };
};

const applyMomoResultToOrder = async (order, payload) => {
  if (!order) return { EC: 1, EM: "KhÃ´ng tÃ¬m tháº¥y Ä‘Æ¡n hÃ ng MoMo", DT: null };
  if (order.paymentMethod !== PAYMENT_METHOD.MOMO) {
    return { EC: 1, EM: "ÄÆ¡n hÃ ng khÃ´ng dÃ¹ng MoMo", DT: decorateOrder(order) };
  }

  const resultCode = Number(payload.resultCode);
  if (!Number.isFinite(resultCode)) {
    return { EC: 1, EM: "MoMo khÃ´ng tráº£ vá» mÃ£ tráº¡ng thÃ¡i há»£p lá»‡", DT: decorateOrder(order) };
  }
  ...
  if ([1000, 7000, 7002, 9000].includes(resultCode) && order.paymentStatus === "PENDING") {
    ...
    order.timeline.push(createTimelineEntry(order.status, payload.message || "MoMo váº«n Ä‘ang chá» xá»­ lÃ½ giao dá»‹ch."));
    await order.save();
    return { EC: 0, EM: "MoMo váº«n Ä‘ang chá» ngÆ°á»i dÃ¹ng xÃ¡c nháº­n thanh toÃ¡n", DT: decorateOrder(order) };
  }

  if (order.paymentStatus === "PAID" || order.paymentStatus === "REFUNDED") {
    return { EC: 0, EM: "ÄÆ¡n Ä‘Ã£ cÃ³ tráº¡ng thÃ¡i thanh toÃ¡n cuá»‘i cÃ¹ng", DT: decorateOrder(order) };
  }

  const note = payload.message
    ? `Thanh toÃ¡n MoMo khÃ´ng thÃ nh cÃ´ng: ${payload.message}`
    : "Thanh toÃ¡n MoMo khÃ´ng thÃ nh cÃ´ng.";
  const updated = await applyFailedMomoResult(order, payload, note);
  return { EC: 0, EM: "ÄÃ£ ghi nháº­n thanh toÃ¡n MoMo khÃ´ng thÃ nh cÃ´ng", DT: decorateOrder(updated) };
};
...
const buildMomoResultPayloadFromQuery = (order, data = {}) => ({
  ...
  orderInfo: data.orderInfo || `Thanh toÃ¡n Ä‘Æ¡n ${order.orderCode}`,
...
const expirePendingMomoOrder = async (order) => {
  ...
  const now = new Date();
  const note = `QuÃ¡ háº¡n thanh toÃ¡n MoMo sau ${MOMO_PAYMENT_EXPIRE_MINUTES} phÃºt. Há»‡ thá»‘ng tá»± há»§y Ä‘Æ¡n, hoÃ n kho vÃ  Ä‘Æ°a sáº£n pháº©m vá» láº¡i giá» hÃ ng.`;
  ...
  order.paymentInfo = {
    ...
    message: queryData.message || "Háº¿t háº¡n thanh toÃ¡n ná»™i bá»™.",
...
const refundPaidMomoOrder = async (order, note) => {
  ...
  if (!transId) {
    return {
      ok: false,
      message: "ÄÆ¡n MoMo Ä‘Ã£ thanh toÃ¡n nhÆ°ng chÆ°a cÃ³ mÃ£ giao dá»‹ch MoMo Ä‘á»ƒ hoÃ n tiá»n",
    };
  }

  const description = note || `HoÃ n tiá»n Ä‘Æ¡n ${order.orderCode}`;
  ...
  if (!refundResult.ok || Number(data.resultCode) !== 0) {
    ...
    return {
      ok: false,
      message: data.message || "MoMo sandbox tá»« chá»‘i hoÃ n tiá»n",
    };
  }
...
export const checkoutService = async (userId, payload = {}) => {
  let reservedItems = [];
  let orderWithCommittedBenefits = null;
  ...
  try {
    ...
    if (hasSelection && !selectedIds.length) {
      return { EC: 1, EM: "Vui lÃ²ng chá»n Ã­t nháº¥t má»™t sáº£n pháº©m Ä‘á»ƒ thanh toÃ¡n", DT: null };
    }
    ...
    if (!validItems.length) {
      return {
        EC: 1,
        EM: selectedSet ? "KhÃ´ng tÃ¬m tháº¥y sáº£n pháº©m Ä‘Ã£ chá»n trong giá» hÃ ng" : "Giá» hÃ ng Ä‘ang trá»‘ng",
        DT: null,
      };
    }
    ...
    for (const item of validItems) {
      if (item.quantity > item.product.stock) {
        return {
          EC: 1,
          EM: `${item.product.name} chá»‰ cÃ²n ${item.product.stock} sáº£n pháº©m trong kho`,
          DT: null,
        };
      }
    }
    ...
    const benefits = await prepareCheckoutBenefitsService(userId, {
      voucherCode: payload.voucherCode,
      pointsToUse: payload.pointsToUse,
      subtotal,
      shippingFee,
    });
    if (benefits.error) return { EC: 1, EM: benefits.error, DT: null };
    const total = Math.round(benefits.total);

    if (payment.method === PAYMENT_METHOD.MOMO && (total < MOMO_MIN_AMOUNT || total > MOMO_MAX_AMOUNT)) {
      return {
        EC: 1,
        EM: `MoMo sandbox chá»‰ nháº­n Ä‘Æ¡n tá»« ${MOMO_MIN_AMOUNT.toLocaleString("vi-VN")} Ä‘áº¿n ${MOMO_MAX_AMOUNT.toLocaleString("vi-VN")} VND`,
        DT: null,
      };
    }
    ...
    const order = await Order.create({
      ...
      subtotal,
      shippingFee,
      voucherCode: benefits.voucherCode,
      voucherTitle: benefits.voucherTitle,
      voucherDiscount: benefits.voucherDiscount,
      pointsUsed: benefits.pointsUsed,
      pointsDiscount: benefits.pointsDiscount,
      total,
      status: ORDER_STATUS.NEW,
      timeline: [
        createTimelineEntry(ORDER_STATUS.NEW, "ÄÆ¡n hÃ ng Ä‘Ã£ Ä‘Æ°á»£c Ä‘áº·t thÃ nh cÃ´ng."),
        createTimelineEntry(
          ORDER_STATUS.NEW,
          isMomo
            ? "ÄÆ¡n Ä‘ang chá» khÃ¡ch hoÃ n táº¥t thanh toÃ¡n MoMo sandbox."
            : "ÄÆ¡n hÃ ng sáº½ Ä‘Æ°á»£c thanh toÃ¡n khi nháº­n hÃ ng."
        ),
      ],
    });

    const benefitsCommit = await commitOrderBenefitsService(order);
    if (!benefitsCommit.ok) {
      order.status = ORDER_STATUS.CANCELLED;
      order.cancelledAt = new Date();
      order.cancelReason = benefitsCommit.message;
      order.benefitsRestoredAt = new Date();
      order.timeline.push(createTimelineEntry(ORDER_STATUS.CANCELLED, benefitsCommit.message));
      await order.save();
      await restoreStock(order.items);
      reservedItems = [];
      return { EC: 1, EM: benefitsCommit.message, DT: null };
    }
    orderWithCommittedBenefits = order;

    if (isMomo) {
      const momoResult = await createMomoPayment({
        orderCode: order.orderCode,
        amount: total,
        orderInfo: `Thanh toÃ¡n Ä‘Æ¡n ${order.orderCode}`,
      ...
      if (!momoResult.ok || Number(momoData.resultCode) !== 0 || !momoData.payUrl) {
        await markMomoCreateFailed(order, momoData.message || "KhÃ´ng táº¡o Ä‘Æ°á»£c link thanh toÃ¡n MoMo sandbox.");
        reservedItems = [];
        return { EC: 1, EM: momoData.message || "KhÃ´ng táº¡o Ä‘Æ°á»£c link thanh toÃ¡n MoMo sandbox", DT: null };
      }

      order.timeline.push(createTimelineEntry(ORDER_STATUS.NEW, "ÄÃ£ táº¡o link thanh toÃ¡n MoMo sandbox, chá» khÃ¡ch thanh toÃ¡n."));
      await order.save();
    }
    ...
    reservedItems = [];
    runNotification(notifyNewOrder(order));

    return {
      EC: 0,
      EM: isMomo
        ? "ÄÃ£ táº¡o yÃªu cáº§u thanh toÃ¡n MoMo sandbox"
        : "Äáº·t hÃ ng COD thÃ nh cÃ´ng",
      DT: decorateOrder(order),
    };
  } catch (error) {
    if (reservedItems.length) await restoreStock(reservedItems);
    if (orderWithCommittedBenefits && !orderWithCommittedBenefits.benefitsRestoredAt) {
      orderWithCommittedBenefits.status = ORDER_STATUS.CANCELLED;
      orderWithCommittedBenefits.cancelledAt = new Date();
      orderWithCommittedBenefits.cancelReason = "Há»‡ thá»‘ng hoÃ n Æ°u Ä‘Ã£i vÃ¬ checkout khÃ´ng hoÃ n táº¥t.";
      orderWithCommittedBenefits.timeline.push(createTimelineEntry(ORDER_STATUS.CANCELLED, orderWithCommittedBenefits.cancelReason));
      await orderWithCommittedBenefits.save();
      await restoreOrderBenefitsService(orderWithCommittedBenefits);
    }
    console.error("checkoutService error:", error);
    return { EC: -1, EM: "Lá»—i server khi thanh toÃ¡n Ä‘Æ¡n hÃ ng", DT: null };
  }
};
...
export const getOrdersService = async (userId) => {
  try {
    ...
    return {
      EC: 0,
      EM: "Láº¥y lá»‹ch sá»­ Ä‘Æ¡n hÃ ng thÃ nh cÃ´ng",
      DT: refreshed.map(decorateOrder),
    };
  } catch (error) {
    console.error("getOrdersService error:", error);
    return { EC: -1, EM: "Lá»—i server khi láº¥y Ä‘Æ¡n hÃ ng", DT: null };
  }
};

export const getOrderDetailService = async (userId, orderId) => {
  try {
    if (!isValidObjectId(orderId)) return { EC: 1, EM: "ÄÆ¡n hÃ ng khÃ´ng há»£p lá»‡", DT: null };
    const order = await Order.findOne({ _id: orderId, user: userId });
    if (!order) return { EC: 1, EM: "KhÃ´ng tÃ¬m tháº¥y Ä‘Æ¡n hÃ ng", DT: null };
    const refreshed = await refreshOrderLifecycle(order);
    return { EC: 0, EM: "Láº¥y chi tiáº¿t Ä‘Æ¡n hÃ ng thÃ nh cÃ´ng", DT: decorateOrder(refreshed) };
  } catch (error) {
    console.error("getOrderDetailService error:", error);
    return { EC: -1, EM: "Lá»—i server khi láº¥y chi tiáº¿t Ä‘Æ¡n hÃ ng", DT: null };
  }
};
...
export const cancelOrderService = async (userId, orderId, reason = "") => {
  try {
    ...
    const note = reason?.trim() || "KhÃ¡ch hÃ ng muá»‘n há»§y Ä‘Æ¡n.";
    ...
    if (policy.action === "direct") {
      ...
      order.timeline.push(createTimelineEntry(ORDER_STATUS.CANCELLED, `KhÃ¡ch hÃ ng há»§y trá»±c tiáº¿p: ${note}`));
      await order.save();
      emitAdminAnalyticsRefresh("ORDER_CANCELLED_BY_USER");
      await restoreStock(order.items);
      await restoreOrderBenefitsService(order);
      if (order.paymentMethod === PAYMENT_METHOD.MOMO) {
        await restoreCartItems(order.user, order.items);
      ...
      return {
        EC: 0,
        EM: "ÄÃ£ há»§y Ä‘Æ¡n hÃ ng trong thá»i gian cho phÃ©p",
    ...
    if (policy.action === "request") {
      ...
      order.cancelReason = note;
      order.timeline.push(createTimelineEntry(ORDER_STATUS.CANCEL_REQUESTED, `KhÃ¡ch hÃ ng gá»­i yÃªu cáº§u há»§y: ${note}`));
      await order.save();
      runNotification(notifyCancelRequest(order));

      return {
        EC: 0,
        EM: "ÄÃ£ gá»­i yÃªu cáº§u há»§y Ä‘Æ¡n cho shop duyá»‡t",
    ...
    return { EC: -1, EM: "Lá»—i server khi há»§y Ä‘Æ¡n hÃ ng", DT: null };
  }
};
...
export const updateOrderStatusService = async (orderId, nextStatus, note = "") => {
  try {
    ...
    if (!Object.values(ORDER_STATUS).includes(nextStatus)) {
      return { EC: 1, EM: "Tráº¡ng thÃ¡i Ä‘Æ¡n hÃ ng khÃ´ng há»£p lá»‡", DT: null };
    }
    ...
    if (!allowed.includes(nextStatus)) {
      return {
        EC: 1,
        EM: `KhÃ´ng thá»ƒ chuyá»ƒn tá»« "${ORDER_STATUS_LABELS[order.status]}" sang "${ORDER_STATUS_LABELS[nextStatus]}"`,
        DT: decorateOrder(order),
      };
    }
    ...
    if ([ORDER_STATUS.CONFIRMED, ORDER_STATUS.PREPARING, ORDER_STATUS.SHIPPING].includes(nextStatus) && !canFulfillOrder(order)) {
      return {
        EC: 1,
        EM: "ÄÆ¡n MoMo chÆ°a thanh toÃ¡n thÃ nh cÃ´ng nÃªn khÃ´ng thá»ƒ chuyá»ƒn sang bÆ°á»›c xá»­ lÃ½/giao hÃ ng",
        DT: decorateOrder(order),
      };
    }
    ...
    if (order.status === ORDER_STATUS.CANCELLED && nextStatus === ORDER_STATUS.CANCELLED && !requiresMomoRefund(order)) {
      return { EC: 1, EM: "ÄÆ¡n Ä‘Ã£ há»§y vÃ  khÃ´ng cÃ²n thao tÃ¡c cáº§n xá»­ lÃ½", DT: decorateOrder(order) };
    }
    ...
    if (nextStatus === ORDER_STATUS.CANCELLED) {
      ...
      if (requiresMomoRefund(order)) {
        const refund = await refundPaidMomoOrder(order, statusNote || `Há»§y vÃ  hoÃ n tiá»n Ä‘Æ¡n ${order.orderCode}`);
        if (!refund.ok) {
          order.paymentStatus = "REFUND_PENDING";
          order.timeline.push(createTimelineEntry(
            order.status,
            `ChÆ°a thá»ƒ há»§y Ä‘Æ¡n vÃ¬ hoÃ n tiá»n MoMo chÆ°a thÃ nh cÃ´ng: ${refund.message}`
          ));
          await order.save();
          return {
            EC: 1,
      ...
      order.cancelReason = statusNote || order.cancelReason || "Shop há»§y/xÃ¡c nháº­n há»§y Ä‘Æ¡n.";
    }
    ...
    if (order.status === ORDER_STATUS.CANCEL_REQUESTED && nextStatus === ORDER_STATUS.PREPARING) {
      ...
      order.timeline.push(createTimelineEntry(
        finalStatus,
        statusNote || "Shop tá»« chá»‘i yÃªu cáº§u há»§y vÃ  tiáº¿p tá»¥c xá»­ lÃ½ Ä‘Æ¡n hÃ ng."
      ));
      order.cancelRequestFromStatus = "";
    } else {
      order.timeline.push(createTimelineEntry(
        nextStatus,
        statusNote || `Cáº­p nháº­t tráº¡ng thÃ¡i: ${ORDER_STATUS_LABELS[nextStatus]}`
      ));
    }
    ...
    if (nextStatus === ORDER_STATUS.DELIVERED && order.paymentMethod === PAYMENT_METHOD.COD) {
      ...
      order.paymentInfo = {
        ...
        paidAt: order.paymentInfo?.paidAt || new Date(),
        message: "ÄÃ£ thu tiá»n COD khi giao hÃ ng thÃ nh cÃ´ng.",
      };
    }

    await order.save();
    if (nextStatus === ORDER_STATUS.DELIVERED) await awardDeliveryLoyaltyService(order);
    runNotification(notifyOrderStatusChanged(order, previousStatus));
    emitAdminAnalyticsRefresh("ORDER_STATUS_UPDATED");
    return { EC: 0, EM: "Cáº­p nháº­t tráº¡ng thÃ¡i Ä‘Æ¡n hÃ ng thÃ nh cÃ´ng", DT: decorateOrder(order) };
  } catch (error) {
    console.error("updateOrderStatusService error:", error);
    return { EC: -1, EM: "Lá»—i server khi cáº­p nháº­t tráº¡ng thÃ¡i", DT: null };
  }
};
...
export const handleMomoReturnService = async (userId, payload = {}) => {
  try {
    if (!verifyMomoResultSignature(payload)) {
      return { EC: 1, EM: "Chá»¯ kÃ½ MoMo khÃ´ng há»£p lá»‡", DT: null };
    }
    ...
    return { EC: -1, EM: "Lá»—i server khi xá»­ lÃ½ káº¿t quáº£ MoMo", DT: null };
  }
};
...
export const handleMomoIpnService = async (payload = {}) => {
  try {
    if (!verifyMomoResultSignature(payload)) {
      return { EC: 1, EM: "Chá»¯ kÃ½ IPN MoMo khÃ´ng há»£p lá»‡", DT: null };
    }
    ...
    return { EC: -1, EM: "Lá»—i server khi xá»­ lÃ½ IPN MoMo", DT: null };
  }
};
...
export const syncMomoPaymentService = async (userId, orderId) => {
  try {
    ...
    if (!queryResult.ok) {
      return { EC: 1, EM: data.message || "KhÃ´ng kiá»ƒm tra Ä‘Æ°á»£c tráº¡ng thÃ¡i MoMo", DT: decorateOrder(order) };
    }
    if (data.resultCode === undefined || data.resultCode === null) {
      return { EC: 1, EM: data.message || "MoMo khÃ´ng tráº£ vá» mÃ£ tráº¡ng thÃ¡i giao dá»‹ch", DT: decorateOrder(order) };
    }
    ...
    return { EC: -1, EM: "Lá»—i server khi kiá»ƒm tra MoMo", DT: null };
  }
};
...
export const sweepExpiredMomoPaymentsService = async () => {
  try {
    ...
    if (!orders.length) return { EC: 0, EM: "KhÃ´ng cÃ³ Ä‘Æ¡n MoMo quÃ¡ háº¡n", DT: { scanned: 0, expired: 0 } };
    ...
    return { EC: 0, EM: "ÄÃ£ quÃ©t Ä‘Æ¡n MoMo quÃ¡ háº¡n", DT: { scanned: orders.length, expired } };
  } catch (error) {
    console.error("sweepExpiredMomoPaymentsService error:", error);
    return { EC: -1, EM: "Lá»—i server khi quÃ©t Ä‘Æ¡n MoMo quÃ¡ háº¡n", DT: null };
  }
};
...
export const getAdminOrdersService = async (query = {}) => {
  try {
    ...
    return {
      EC: 0,
      EM: "Láº¥y danh sÃ¡ch Ä‘Æ¡n hÃ ng admin thÃ nh cÃ´ng",
      DT: {
    ...
    return { EC: -1, EM: "Lá»—i server khi láº¥y Ä‘Æ¡n hÃ ng admin", DT: null };
  }
};
...
export const getAdminDashboardService = async () => {
  try {
    ...
    const [members, activeVouchers, reviews] = await Promise.all([
      User.countDocuments({ role: "user" }),
      Voucher.countDocuments({ active: true, startAt: { $lte: now }, endAt: { $gte: now } }),
    ...
    return {
      EC: 0,
      EM: "Láº¥y dashboard admin thÃ nh cÃ´ng",
      DT: {
        totals: {
    ...
    return { EC: -1, EM: "Lá»—i server khi láº¥y dashboard admin", DT: null };
  }
};
